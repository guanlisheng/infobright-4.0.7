...
"section of the manual.");
      ...
      */
      ...
      if (!(res= open_and_lock_tables_derived(thd, lex->query_tables, TRUE, 1)))
      {
        /*
...
end_with_restore_list:
  ...
  */
  ...
  case SQLCOM_INSERT_SELECT:
  {
    ...
    if (!(res= open_and_lock_tables_derived(thd, all_tables, TRUE, 1)))
    {
...
static bool execute_sqlcom_select(THD *thd, TABLE_LIST *all_tables)
{
  ...
  // open_and_lock_tables does not fill derived tables before a call to BrightHouse engine
  // if (!(res= open_and_lock_tables(thd, all_tables)))
  if (!(res= open_and_lock_tables_derived(thd, all_tables, TRUE, 1))
	  && (!lex->describe || !(res= open_and_lock_tables_derived(thd, all_tables, TRUE, 2))))
  {
    if (lex->describe)
    {
      /*
