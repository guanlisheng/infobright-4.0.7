...
#include "sslopt-longopts.h"
  ...
  {"version", 'V', "Output version information and exit.", 0, 0, 0, GET_NO_ARG,
   NO_ARG, 0, 0, 0, 0, 0, 0},
  {"infobright-version", OPT_INFOBRIGHT_VERSION, "Output Infobright version information and exit.", 0, 0, 0, GET_NO_ARG,
   NO_ARG, 0, 0, 0, 0, 0, 0},
...
static void print_infobright_version(void)
{
	printf("%d\n%s\n", CURRENT_DATA_VERSION, InfobrightServerVersion());
}
...
#include <sslopt-case.h>
  ...
  case 'V':
    print_version();
    exit(0);
	case OPT_INFOBRIGHT_VERSION:
		print_infobright_version();
		exit(0);
#endif /*EMBEDDED_LIBRARY*/
  case 'W':
    if (!argument)
