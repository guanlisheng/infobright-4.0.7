...
bool handle_select(THD *thd, LEX *lex, select_result *result,
                   ulong setup_tables_done_option, int optimize_after_bh, int free_join_from_bh)
{
  bool res;
  ...
  {
	  res = FALSE;
	  if (optimize_after_bh) res= lex->unit.optimize_after_brighthouse();   // optimization after BrightHouse
	  if (!res) res= mysql_union(thd, lex, result, &lex->unit, setup_tables_done_option);
  }
  else
  {
...
int
JOIN::optimize(unsigned char part)
{
  ...
  */
  ...
  conds= optimize_cond(this, conds, join_list, &cond_value, part);   
  if (thd->is_error())
  {
    error= 1;
  ...
  {
    having= optimize_cond(this, having, join_list, &having_value, part);
    if (thd->is_error())
    {
      error= 1;
      DBUG_PRINT("error",("Error from optimize_cond"));
    ...
    if (cond_value == Item::COND_FALSE || having_value == Item::COND_FALSE || 
        (!unit->select_limit_cnt && !(select_options & OPTION_FOUND_ROWS)) || optimize_completed)
    {						/* Impossible cond */
  ...
  if (part==3)
	  for (TABLE_LIST* tables = select_lex->leaf_tables; tables; tables = tables->next_leaf)
	  {
		  if (tables->table->file && tables->table->file->ht && tables->table->file->ht->db_type==DB_TYPE_HEAP
			  && ((ha_heap*)tables->table->file)->get_hp_info() && ((ha_heap*)tables->table->file)->get_hp_info()->s && ((ha_heap*)tables->table->file)->get_hp_info()->s->records==0)
			  ((ha_heap*)tables->table->file)->get_hp_info()->s->records = 2;
	  }
  ...
  if (part==3)
	  ...
	  {
		  if (tables->table->file && tables->table->file->ht && tables->table->file->ht->db_type==DB_TYPE_HEAP
			  && ((ha_heap*)tables->table->file)->get_hp_info() && ((ha_heap*)tables->table->file)->get_hp_info()->s && ((ha_heap*)tables->table->file)->get_hp_info()->s->records==2)
			  ((ha_heap*)tables->table->file)->get_hp_info()->s->records = 0;
	  }
  ...
  if (const_tables && !thd->locked_tables &&
      !(select_options & SELECT_NO_UNLOCK) && part!=3)
...
else
{
  ...
  if (optimize_after_bh)			// optimization after BrightHouse
  {
	  int part = optimize_after_bh == TRUE ? 2 : 4;
	   if(err= join->optimize(part))
		goto err;					// 1
  }
  else if (!optimize_after_bh && (err= join->optimize()))
  {
    goto err;					// 1
  }
...
make_join_readinfo(JOIN *join, ulonglong options)
{
  ...
  // GA : make sure that the assumption is correct
  if (join->tables) {
...
optimize_cond(JOIN *join, COND *conds, List<TABLE_LIST> *join_list,
              Item::cond_result *cond_value, unsigned char part)
{
  THD *thd= join->thd;
  DBUG_ENTER("optimize_cond");

  if (!conds || 1 == 1) // TODO: Infobright: switching off to see the result
  ...
  else
  {
    ...
    */
    ...
    conds= remove_eq_conds(thd, conds, cond_value, part) ;
...
COND *
remove_eq_conds(THD *thd, COND *cond, Item::cond_result *cond_value, unsigned char part)
{
  if (cond->type() == Item::COND_ITEM)
  {
    bool and_level= ((Item_cond*) cond)->functype()
    ...
    *cond_value=Item::COND_UNDEF;
    ...
    while ((item=li++))
    {
      Item *new_item=remove_eq_conds(thd, item, &tmp_cond_value, part);
      if (!new_item)
	li.remove();
