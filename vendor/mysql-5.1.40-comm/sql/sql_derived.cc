...
bool
mysql_handle_derived(LEX *lex, bool (*processor)(THD*, LEX*, TABLE_LIST*), List<st_select_lex_unit>* derived_optimized)
{
  bool res= FALSE;
  if (lex->derived_tables)
  {
    List_iterator_fast<st_select_lex_unit> bh_it;
	if (derived_optimized) bh_it.init(*derived_optimized);
	 ...
	 sl= sl->next_select_in_list())
    {
      ...
      {
	if (derived_optimized)
	{
		// Checks whether the derived table was optimized by BrightHouse
		bh_it.rewind();
		st_select_lex_unit* found;
		while (found = bh_it++)
		  if (found==cursor->derived) break;
		if (found && (res = cursor->derived->optimize_after_brighthouse()))
		  goto out;
	}
